...
LabChartdir = fullfile(ResDir,subno,'ProcessedLC');
LabChartMat = dir(fullfile(LabChartdir,'*LabChart*.mat'));
...
NROutdir = fullfile(ResDir, subno, 'NuisanceRegressor');
...
%         plot(find(adj_slicevar{ss, r}> crit(2)), adj_slicevar{ss,r}(adj_slicevar{ss, r}> crit(2)), '*r');
        ...
        NRfilename = fullfile(NROutdir,sprintf('Nz_nuisance_regressors_%s_%s_%s.txt', subno, sess_name, run_name));
        dlmwrite(NRfilename, NR{ss,r}, 'delimiter', ' ', 'precision', '%.24f');   %not doing z scoring!!!
    end % end of runs
