...
% This code redo the step for the model without '_redo',
% with the important change that it saves the residuals
% and it uses the 3mm voxel size, for the ease of calculation later

%% setup
fprintf('Specifying and estimating GLM for %s\n',subno);
External = '/Volumes/QF10TB/SPEEDTMS_results';

modelname = 'iPE_fourpt_clean_semiconcat_noResp_Nz_redo';
...
for r = 1:nruns
    rctr = rctr + 1;
    scandir = fullfile(External,'TempFuncCopied',subno,'func',sess_name,sprintf('Run%d',r));
    filenames = dir(fullfile(scandir,'s6mmw3f*.nii')); 
