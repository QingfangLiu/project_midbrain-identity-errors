...
%% read ROI idx 
roiname = {'Conj_p5_MB_b.nii','LPFC_b.hdr'};
clear roiidx

for i = 1:length(roiname)
    roi_vol = spm_read_vols(spm_vol(fullfile(maskdir,'WB_corrected_masks',roiname{i})));
...
beta_file_name = sprintf('Betas_%s_WB.mat',modelname); % updated with WB version
