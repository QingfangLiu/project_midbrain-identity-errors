...
function Do_8_function_GroupGLM_OneSample_Ttest_ChangeEm(modelname,connames)
...
Groupdir = fullfile(studydir,'GroupRes'); % create a folder to store group analysis result
...
ModelDir = fullfile(Groupdir,sprintf('%s_changeEm',modelname)); % create a model directory
...
for c = 1:length(connames)
    ...
    for j = 1:nSubs
        tempConImage = fullfile(studydir,sprintf('Sub%d',Subs(j)),'fxUnivariate',modelname,sprintf('con_%04d.nii', c));
    ...
    maskmap = fullfile(parentDir,'ROIs','SeedRegion_OFC_bilateral.nii');
    ...
    %% save the job
    fname = fullfile(Jobdir, sprintf('Group_Specify_Univ_%s_%s.mat',sprintf('%s_ChangeEm',modelname),connames{c}));
