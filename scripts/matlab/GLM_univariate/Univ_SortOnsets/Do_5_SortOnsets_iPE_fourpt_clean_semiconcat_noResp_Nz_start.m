...
% load the odor inhale start time onsets
load(fullfile(subdir,sprintf('StartInhaleTimeOnsets_%s.mat',subno)))
...
for r = 1:nruns
    ...
    InhaleStartOnsets = FinalTimes(:,rctr+1);
    InhaleStartOnsets = InhaleStartOnsets(~isnan(InhaleStartOnsets));
    d = [d,InhaleStartOnsets];
