...
fprintf('Working on sorting onsets iPE fourpt pseudoconcat noResp for %s\n',subno);
...
for i = 1:24 % 4 for each run, 6 runs in total
    onsets{i} = [];
end
...
for r = 1:nruns
    ...
    %tmpons{6} = (d(~isnan(d(:,6)),8) + d(~isnan(d(:,6)),6) + timecorr)./1000; % response timing
    ...
    onsets{rctr*4+1} = tmpons{1};  % reversal onsets
    onsets{rctr*4+2} = tmpons{2};  % non-reversal minus1 onsets
    onsets{rctr*4+3} = tmpons{3};  % non-reversal plus1 onsets
    onsets{rctr*4+4} = tmpons{4};  % non-reversal plus2 onsets
    
    % concatenate cues and resps onsets across all runs
    cues_ons = [cues_ons; tmpons{5}];
    %resp_ons = [resp_ons; tmpons{6}];
