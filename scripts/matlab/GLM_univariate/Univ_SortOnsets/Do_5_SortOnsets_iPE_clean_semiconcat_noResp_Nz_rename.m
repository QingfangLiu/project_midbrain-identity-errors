...
% the scan timing.
% The events include: cue onset, reversal prediction, all non-reversal prediction.

% This is for a semiconcatenate model.
% To estimate two regressors of interest (reversal, non-reversal) for each
% of three runs from the same day, but concatenate other two regressors (cues, response), and 34
% nuisance regressors.
...
for i = 1:4 % 2 for each session, 2 sessions in total
    onsets{i} = [];
end
...
for r = 1:nruns
    ...
    fprintf('%s %s\n',run_name,sess_name)
    rundir = fullfile(subdir,'func',sess_name,run_name);
    ...
    tmpons{1} = (d(d(:,3)==1 & ~choice_acc,11) + timecorr)./1000; % reversal % wrong acc only
    tmpons{2} = (d(d(:,3)==0 & choice_acc,11) + timecorr)./1000; % non-reversal % correct acc only
    tmpons{3} = (d(:,7) + timecorr)./1000; % cue presentation timing

    % adjust onset timings for runs from the second run
    for i = 1:length(tmpons)
        tmpons{i} = tmpons{i} + nvols * TR * rctr;
    end
            
    % keep reversal and non-reversal onsets for current run
    % updated here: separate onsets for the first 3 runs and later 3 runs
    onsets{ssctr*2+1} = [onsets{ssctr*2+1}; tmpons{1}];  % reversal onsets
    onsets{ssctr*2+2} = [onsets{ssctr*2+2}; tmpons{2}];  % non-reversal onsets
    
    % concatenate cues onsets across all runs
    cues_ons = [cues_ons; tmpons{3}];
    rctr = rctr + 1;  % count up run idx

end  % end of the run

    %names = [names name_tmp]; % add condition names after each session
...
names = [{'Outcomes_Reversal_S1'} {'Outcomes_NonRev_S1'} ...
    {'Outcomes_Reversal_S2'} {'Outcomes_NonRev_S2'} ...
    {'cues'}]; 
