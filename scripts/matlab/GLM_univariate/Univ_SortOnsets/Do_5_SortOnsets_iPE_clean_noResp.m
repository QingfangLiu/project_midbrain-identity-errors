...
% This GLM is modified from iPE model, but only includes trials with
% correct responses for non-reversal trials and wrong responses for
% reversal trials.

%% initialize things

fprintf('Working on sorting onsets iPE clean noResp for %s\n',subno);
...
for r = 1:nruns
    ...
    NoniPE_outcomes = (d(d(:,3)==0 & choice_acc,11) + timecorr)./1000; % correct acc only
    cues = (d(:,7) + timecorr)./1000; % cue presentation timing
    %responses = (d(~isnan(d(:,6)),8) + d(~isnan(d(:,6)),6) + timecorr)./1000; % response timing

    names{1} = 'Outcomes_Reversal';
    names{2} = 'Outcomes_NonRev';
    names{3} = 'Cues';
    %names{4} = 'Responses';

    onsets{1} = iPE_outcomes;
    onsets{2} = NoniPE_outcomes; 
    onsets{3} = cues;
    %onsets{4} = responses;
