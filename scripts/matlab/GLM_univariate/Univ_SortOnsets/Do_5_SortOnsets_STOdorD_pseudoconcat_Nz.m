...
fprintf('Sorting onsets on %s\n',subno);

% set a new model directory
modelname = 'STOdorD_pseudoconcat_Nz'; % single trial odor delivery
...
clear onsets durations NRs badvols
rctr = 0; % count runs
trialctr = 0; % count trials across runs and sessions
...
for r = 1:nruns % runs
    ...
    for t = 1:size(d,1)
        trialctr = trialctr + 1;
        onsets{trialctr} = (d(t,11) + timecorr)./1000 + nvols * TR * rctr;
    end
...
names = cell(1,trialctr);
for t = 1:trialctr
    names{t} = sprintf('O%d',t);
end
