...
modelname = 'Mixed_fourpt_clean_semiconcat_noResp_Nz';
...
% condition names
names = [];
name_tmp = [{'Expect_Reversal'} {'Expect_Minus1'} {'Expect_Plus1'} {'Expect_Plus2'},...
    {'Outcome_Reversal'} {'Outcome_Minus1'} {'Outcome_Plus1'} {'Outcome_Plus2'}]; 

for i = 1:16 % 4*2 for each session, 2 sessions in total
    onsets{i} = [];
end

rctr = 0; % count runs
ssctr = 0; % count sessions
%odor_ons = []; % all odor onsets will be in one column
...
for r = 1:nruns
    ...
    tmpons{1} = (d(d(:,3)==1,7) + timecorr)./1000; % reversal at cue present timing
    tmpons{5} = (d(d(:,3)==1,11) + timecorr)./1000; % reversal at odor delivery timing
    ...
    Minus1_expect_cue1 = (cue1d(rev_loc_1 - 1,7) + timecorr)./1000;
    Minus1_expect_cue1 = Minus1_expect_cue1(choice_acc_1(rev_loc_1 - 1)); % correct acc only
    ...
    Plus1_expect_cue1 = (cue1d(rev_loc_1 + 1,7) + timecorr)./1000;
    Plus1_expect_cue1 = Plus1_expect_cue1(choice_acc_1(rev_loc_1 + 1)); % correct acc only
    ...
    Plus2_expect_cue1 = (cue1d(rev_loc_1 + 2,7) + timecorr)./1000;
    Plus2_expect_cue1 = Plus2_expect_cue1(choice_acc_1(rev_loc_1 + 2)); % correct acc only
    ...
    Minus1_expect_cue2 = (cue2d(rev_loc_2 - 1,7) + timecorr)./1000;
    Minus1_expect_cue2 = Minus1_expect_cue2(choice_acc_2(rev_loc_2 - 1)); % correct acc only
    ...
    Plus1_expect_cue2 = (cue2d(rev_loc_2 + 1,7) + timecorr)./1000;
    Plus1_expect_cue2 = Plus1_expect_cue2(choice_acc_2(rev_loc_2 + 1)); % correct acc only
    ...
    Plus2_expect_cue2 = (cue2d(rev_loc_2 + 2,7) + timecorr)./1000;
    Plus2_expect_cue2 = Plus2_expect_cue2(choice_acc_2(rev_loc_2 + 2)); % correct acc only
    ...
    tmpons{2} = [Minus1_expect_cue1;Minus1_expect_cue2];
    tmpons{3} = [Plus1_expect_cue1;Plus1_expect_cue2];
    tmpons{4} = [Plus2_expect_cue1;Plus2_expect_cue2];
    
    tmpons{6} = [Minus1_outcomes_cue1;Minus1_outcomes_cue2];
    tmpons{7} = [Plus1_outcomes_cue1;Plus1_outcomes_cue2];
    tmpons{8} = [Plus2_outcomes_cue1;Plus2_outcomes_cue2];
    ...
    onsets{ssctr*8+1} = [onsets{ssctr*8+1}; tmpons{1}];  % reversal onsets
    onsets{ssctr*8+2} = [onsets{ssctr*8+2}; tmpons{2}];  % non-reversal minus1 onsets
    onsets{ssctr*8+3} = [onsets{ssctr*8+3}; tmpons{3}];  % non-reversal plus1 onsets
    onsets{ssctr*8+4} = [onsets{ssctr*8+4}; tmpons{4}];  % non-reversal plus2 onsets
    
    onsets{ssctr*8+5} = [onsets{ssctr*8+5}; tmpons{5}];  % reversal onsets
    onsets{ssctr*8+6} = [onsets{ssctr*8+6}; tmpons{6}];  % non-reversal minus1 onsets
    onsets{ssctr*8+7} = [onsets{ssctr*8+7}; tmpons{7}];  % non-reversal plus1 onsets
    onsets{ssctr*8+8} = [onsets{ssctr*8+8}; tmpons{8}];  % non-reversal plus2 onsets
    
    % concatenate odor delivery onsets across all runs
    %odor_ons = [odor_ons; tmpons{5}];    
...
% add names of odor delivery conditions
%names = [names {'odor'}];

% add other two onsets one by one after reversal trial onsets
%onsets{length(onsets)+1} = odor_ons;
