...
modelname = 'Revs_pseudoconcat_Nz_Ctrl';
...
PE_ons = []; % all reversal outcomes
nonPE_ons = []; % all other non reversal outcomes will be in one column
...
for r = 1:nruns % runs
    ...
    % Change the way to reorganize matrix d: not putting reversal labels on the 
    % reversal trials, but on the trials after.
    ...
    for i = 1:2 %loop through 2 cue types
        ...
        dtmp(find(dtmp(:,3)==1)+1,3) = rtype; % put reversal labels into post-reversal trials
        Newd = [Newd; dtmp];
    end
    ...
    for rev = rtypes
        ...
        onsets{revctr} = (Newd(Newd(:,3)==rev,11) + timecorr)./1000 + nvols * TR * rctr;
    end

    % all reversal trials
    PE_ons = [PE_ons; ((d(d(:,3)==1,11) + timecorr)./1000 + nvols * TR * rctr)];
    
    % other non-reversal trials
    nonPE_ons = [nonPE_ons; ((Newd(Newd(:,3)==0,11) + timecorr)./1000 + nvols * TR * rctr)];
