...
modelname = 'SRevs_pseudoconcat_Nz'; % single reversals
...
name_tmp = [{'1->2'} {'1->2'} {'1->3'} {'1->3'} {'2->1'} {'2->1'} ...
...
for r = 1:nruns % runs
    ...
    for rev = rtypes
        ...
        onsets{revctr} = (d(d(:,3)==rev & d(:,1)==1,11) + timecorr)./1000 + nvols * TR * rctr;
        revctr = revctr + 1;
        onsets{revctr} = (d(d(:,3)==rev & d(:,1)==2,11) + timecorr)./1000 + nvols * TR * rctr;
    end
